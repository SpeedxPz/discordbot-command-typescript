// TakumiPro Command.ts version 1.2.1 (2021/12/25)
...
class Argument {
  ...
  setName(name: string, display: string = undefined): Argument {
    ...
    if (!name.match(/^[a-z0-9_]+$/i))
      throw new Error(
        'Argument of setName should contain only chars A-z, 0-9 and _',
      );
  ...
  _validate(arg: string, ...rest: string[]): string[] {
    ...
    if (this._minlen !== null && this._minlen > arg.length)
      throw new ParseError(
        `String length not greater or equal! Expected at least ${this._minlen}, but got ${arg.length}`,
        this,
      );
    if (this._maxlen !== null && this._maxlen < arg.length)
      throw new ParseError(
        `String length not less or equal! Maximum ${this._maxlen} chars allowed, but got ${arg.length}`,
        this,
      );
    if (this._whitelist !== null && !this._whitelist.includes(arg))
      throw new ParseError(
        `Invalid Input for ${arg}. Allowed words: ${this._whitelist.join(
          ', ',
        )}`,
        this,
      );
    if (this._regex !== null && !this._regex.test(arg))
      throw new ParseError(
        `Regex missmatch, the input '${arg}' did not match the expression ${this._regex.toString()}`,
        this,
      );
    return [arg, ...rest];
  }
...
class NumberArgument extends Argument {
  ...
  validate(args): string[] {
    ...
    if (this._min !== null && this._min > num)
      throw new ParseError(
        `Number not greater or equal! Expected at least ${this._min}, but got ${num}`,
        this,
      );
    if (this._max !== null && this._max < num)
      throw new ParseError(
        `Number not less or equal! Expected at least ${this._max}, but got ${num}`,
        this,
      );
...
class CommandBase {
  ...
  alias(...alias: string[]): CommandBase {
    ...
    this._alias.push(
      ...alias.filter((a) => this._collector.getAvailableCommands(a)),
    );
    return this;
  }
  ...
  isAllowed(client: any): any {
    return Promise.all(
      this._permissionHandler.map((cb) => cb(client)),
  ...
  async _dispatchCommand(ev: ICommandDispatch): Promise<void> {
    ...
    await Promise.all(
      this._execHandler.map(async (handle: ICommandExecute) =>
        handle(ev.client, ev.arguments, ev.message, ev.cmdUtils),
      ),
    );
    return;
  }
...
class Command extends CommandBase {
  ...
  requireBotPermission(
    perm: Discord.BitFieldResolvable<Discord.PermissionString, bigint>,
  ): Command {
  ...
  getUsage(): string {
    return `${this.getCommandName()} ${this.getArguments()
      .map((arg) => arg.getManual())
      .join(' ')}`;
  }

  getUsageAlias(): string[] {
    return this._alias.map((alias) => {
      return `${alias} ${this.getArguments()
        .map((arg) => arg.getManual())
        .join(' ')}`;
    });
  }
  ...
  async dispatch(
    args: string,
    ev: ICommandMessage,
    cmdUtil: CommandUtility,
  ): Promise<void> {
    const botPermission = ev.message.guild.me.permissionsIn(
      ev.message.channel as Discord.TextChannel,
    );
      ...
      await ev.message.author.send({
        embeds: [
          new MessageEmbed()
            ...
            .setDescription(
              `I didn't have permission to send message on server **${
                ev.message.guild.name
              }** channel **#${
                (ev.message.channel as TextChannel).name
      ...
      await ev.message.channel.send({
        embeds: [
          new MessageEmbed()
            .setTitle(`Error! Missing Bot Permission`)
            .setDescription(
              `Bot require following permission on this server/channel to perform the command`,
            )
...
class CommandCollector {
  ...
  static async checkPermissions(commands: CommandBase[], client: any) {
    const result = await Promise.all(
      commands.map((cmd) => cmd.hasPermission(client)),
    );
  ...
  getAvailableCommands(name: string) {
    name = name.toLowerCase();
    return this._commands.filter((cmd) => cmd.getCommandNames().includes(name));
  }

  getAvailableCommandsByPermission(client: any): Promise<CommandBase[]> {
    return CommandCollector.checkPermissions(this._commands, client);
  }
...
const messageHandler = async (ev: ICommandMessage): Promise<void> => {
  ...
  if (
    ...
    const prefix = await cmdUtil.getCommandPrefix(guildId);
    await ev.message.channel.send(
      `Command prefix is: **${prefix}**\nUse **${prefix}help** to get all available command`,
    );
    return;
  }
  ...
  const match = message.match(
    new RegExp(`^(?<command>\\S*)\\s*(?<args>.*)\\s*$`, 's'),
  );
  if (!match || !match.groups)
    throw new Error(`command regex mismatch for '${message}'`);
...
export const createArgument = (type: string): Argument => {
  ...
  if (!(arg instanceof Argument)) {
    throw new Error(
      `Argument type not supported, Available : ${Object.keys(
        Argument.createArgumentType(),
      ).join(', ')}`,
    );
  }
  return arg;
...
export const getManualEmbed = async (
  client: Discord.Client,
  cmdName: string,
    ...
    try {
      ...
      if (e instanceof PermissionError) {
        await ev.message.reply(
          `
            You don't have permission to use this command\n
            For available command type **${cmdUtil.getCommandPrefix(
              guildId,
            )}help**
          `,
        );
        ...
        await ev.message.reply(
          `
            Invalid command usage!\n
            For usage detail type **${cmdUtil.getCommandPrefix(
              guildId,
...
createCommand('help')
  ...
  .manual(
    `You can specific command name to view how to use (eg. help <command name>)`,
  )
  ...
  .exec(
    async (
      ...
      if (!args.cmdName || args.cmdName == '') {
        const cmdsWithHelp = (
          await collector.getAvailableCommandsByPermission(message)
        ...
        await Promise.all(
          cmdsWithHelp.map(async (cmd: CommandBase) => {
            commands.push([
              `${commandPrefix}${cmd.getCommandName()}`,
              cmd.getHelp(),
            ]);
          }),
        );
        ...
        while (commands.length > 0) {
          ...
          if (embedArrays) {
            const embed: Discord.MessageEmbed = new MessageEmbed()
              ...
              .setDescription(
                `Type \`\`${commandPrefix}help <command name>\`\` for more details`,
              )
              .setColor(4886754)
              .setThumbnail(client.user.avatarURL());
        ...
        const cmds = await CommandCollector.checkPermissions(
          collector.getAvailableCommands(args.cmdName),
          message,
        );
        if (cmds.length === 0) {
